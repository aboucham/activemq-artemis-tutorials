...
- Deploy an AMQ Broker cluster with the console and monitoring properties enabled in the namespace amq. Example file attached: `activemqartemis-ex-aao-rhbk.yaml`. The deployment requires a secret to be created with the name `mytlssecret` for the console to be secured in `HTTPs`:
...
[source, bash]
----
oc apply -n amq -f https://raw.githubusercontent.com/aboucham/activemq-artemis-tutorials/refs/heads/main/activemqartemis-ex-aao-rhbk.yaml
----
